...
def get_mae(nestimates, train_x, test_x, train_y, test_y):
    model = RandomForestClassifier(n_estimators = nestimates, random_state = 1)
...
train_X_meta, test_X_meta, train_Y_meta, test_Y_meta = train_test_split(X_meta, Y, test_size = 0.5, random_state = 1)
...
forest_meta = RandomForestClassifier(n_estimators = n_estimates_meta[1], random_state = 1)
...
mat_ens_no = confusion_matrix(test_Y, preds_ensemble_no)
sns.heatmap(mat_ens_no.T, square=True, annot=True, fmt='d', cbar=False, xticklabels = ["Survival", "Decease"], yticklabels = ["Predicted Survival", "Predicted Decease"])
plt.title("Ensemble Model without Metastasis Data")
plt.show()

mat_ens_meta = confusion_matrix(test_Y_meta, preds_ensemble_meta)
sns.heatmap(mat_ens_meta.T, square=True, annot=True, fmt='d', cbar=False, xticklabels = ["Survival", "Decease"], yticklabels = ["Predicted Survival", "Predicted Decease"])
plt.title("Ensemble Model with Metastasis Data")
plt.show()
...
result = permutation_importance(
    forest_meta, test_X_meta, test_Y_meta, n_repeats=10, random_state=1, n_jobs=2)
...
preds_ensemble_no_train = ensemble_no.predict(train_X)
ensemble_no_accuracy_train = accuracy_score(preds_ensemble_no_train, train_Y)

preds_ensemble_meta_train = ensemble_meta.predict(train_X_meta)
ensemble_meta_accuracy_train = accuracy_score(preds_ensemble_meta_train, train_Y_meta)
